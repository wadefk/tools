...
function! <SID>Gen_makefile()
	...
	if g:lib_mode == "lib"
		exec '!gen_makefile.sh '.code_file_path.' '.base.' so'
	else
		exec '!gen_makefile.sh '.code_file_path.' '.base
	endif
endfunction
...
function! <SID>In_class_public(fun_row, fun_colomn)
	...
	if private_protect_row == 1 || private_protect_row == 0 &&  public_row == left_bracket_row
...
function! <SID>Private_switch()
	if g:private_switch_flag == 0
		echo "** 私有和保护函数测试开启! 此功能会对源代码进行修改，请谨慎使用!"
		echo "** 再次按下该快捷键关闭此功能"
